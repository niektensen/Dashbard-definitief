...
# De gewenste variabelenlijst
base_options_list = [
    'MoeiteMetRondkomen_1', 'HeeftSchulden_3', 'GoedErvarenGezondheid_6',
    'SlaaptMeestalSlecht_7', 'GoedErvarenMentaleGezondheid_12', 'TevredenMetEigenLeven_11',
    'Overgewicht_59', 'SportWekelijks_66', 'AlcoholAfg12Maanden_69',
    'RooktTabak_75', 'CannabisInAfg12Maanden_89', 'ZwareDrinker_72',
    'BeweegtDagelijksMinHalfUur_64', 'OnlineGegoktVoorGeldAfg12Maanden_137',
    'NegatieveGevolgenVanCoronaperiode_14'
]

# Filter de lijst om alleen variabelen te behouden die ook in het DataFrame voorkomen
options_list = [col for col in base_options_list if col in df.columns]

# Definieer de lijsten voor de correlatie-heatmap op basis van de nieuwe lijst
financieel_vars_base = ['MoeiteMetRondkomen_1', 'HeeftSchulden_3', 'OnlineGegoktVoorGeldAfg12Maanden_137']
gezondheid_leefstijl_vars_base = [
    'GoedErvarenGezondheid_6', 'SlaaptMeestalSlecht_7', 'GoedErvarenMentaleGezondheid_12',
    'TevredenMetEigenLeven_11', 'Overgewicht_59', 'SportWekelijks_66',
    'AlcoholAfg12Maanden_69', 'RooktTabak_75', 'CannabisInAfg12Maanden_89',
    'ZwareDrinker_72', 'BeweegtDagelijksMinHalfUur_64', 'NegatieveGevolgenVanCoronaperiode_14'
]
financieel_vars = [col for col in financieel_vars_base if col in df.columns]
gezondheid_leefstijl_vars = [col for col in gezondheid_leefstijl_vars_base if col in df.columns]
...
# --- Scatterplot en Bar Chart in gelijke kolommen ---
...
with col1:
    with st.container(border=True):
        ...
        with col_s1:
            x_axis = st.selectbox('X-as', options_list, index=options_list.index('MoeiteMetRondkomen_1'))
        with col_s2:
            y_axis = st.selectbox('Y-as', options_list, index=options_list.index('HeeftSchulden_3'))
...
with col2:
    with st.container(border=True):
        ...
        selected_bar = st.selectbox('Selecteer Variabele', options_list, index=0)
...
# --- Histogram over de volledige breedte ---
with st.container(border=True):
    st.subheader("Histogram")
    hist_ms = st.multiselect('Selecteer Provincies voor Histogram', provinces, default=provinces[:3], key='hist_ms')
    selected_hist = st.selectbox('X-as Histogram', options_list, index=0, key='hist_select')
    ...
    toon_mediaan = st.checkbox('Toon Mediaan', value=False)
    ...
    if hist_ms:
        ...
        if toon_mediaan:
            # Mediaanlijn over het histogram
            median_line = alt.Chart(df_hist_filtered).mark_rule(color='red', strokeWidth=2).encode(
                x=alt.X(f"{selected_hist}:Q", aggregate='median'),
                tooltip=[alt.Tooltip(f"{selected_hist}:Q", title="Mediaan", aggregate='median', format=".2f")]
            )
            final_chart = hist + median_line
...
# --- Grouped Bar ---
with st.container(border=True):
    numeric_options = [col for col in options_list if pd.api.types.is_numeric_dtype(df[col])]
    stack_vars = st.multiselect(
        'Selecteer Variabelen voor Grouped Bar',
        numeric_options,
        default=numeric_options[:1]
    )
...
map_sb = st.selectbox('Kies variabele voor de kaart', options_list, index=0, key='map_sb')
